...
func serve() error {
	...
	if err != nil { return fmt.Errorf("open canonical management state: %w", err) }
	defer func() {
		if err := state.Close(); err != nil { log.Printf("ERROR: close management state: %v", err) }
	}()

	sources, err := sourcelibrary.New(sourcesDir, embeddedRoot, state, sourcelibrary.NewGithubRemote())
	if err != nil { return fmt.Errorf("open canonical source library: %w", err) }
	...
	server := &http.Server{
		Handler: studio.Handler(identity), ReadHeaderTimeout: 5 * time.Second,
...
func requireLoopbackListener(listener net.Listener) error {
	...
	if !ok || address.IP == nil || !address.IP.IsLoopback() { return fmt.Errorf("refusing non-loopback VPSmith Studio listener %s", listener.Addr()) }
	return nil
}

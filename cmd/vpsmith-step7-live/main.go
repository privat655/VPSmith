...
func (f appFlags) open(ctx context.Context) (*application.Application, error) {
	if *f.stateDir == "" || *f.sourcesDir == "" || *f.runtimeDir == "" {
		return nil, errors.New("state-dir, sources-dir, and runtime-dir are required")
	}
	stateAbs, err := filepath.Abs(*f.stateDir)
	if err != nil {
		return nil, err
	}
	sourcesAbs, err := filepath.Abs(*f.sourcesDir)
	if err != nil {
		return nil, err
	}
	embeddedAbs, err := filepath.Abs(*f.embeddedRoot)
	if err != nil {
		return nil, err
	}
	runtimeAbs, err := filepath.Abs(*f.runtimeDir)
	if err != nil {
		return nil, err
	}
...
func prepare(args []string) error {
	...
	if *output == "" || *targetIDOutput == "" {
		return errors.New("output and target-id-output are required")
	}
	ctx := context.Background()
	app, err := common.open(ctx)
	if err != nil {
		return err
	}
	defer app.Close()
...
func observe(args []string) error {
	...
	if *targetID == "" || *address == "" || *output == "" {
		return errors.New("target-id, address, and output are required")
	}
	...
	if err := app.SetTargetAddress(ctx, id, *address); err != nil {
		return err
	}
	...
	if err := os.WriteFile(*output, data, 0o600); err != nil {
		return fmt.Errorf("write host-key observation: %w", err)
	}
...
func confirm(args []string) error {
	...
	if *targetID == "" || *observationPath == "" {
		return errors.New("target-id and observation are required")
	}
	ctx := context.Background()
	app, err := common.open(ctx)
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("read host-key observation: %w", err)
	}
	var observation targetgateway.HostKeyObservation
	if err := json.Unmarshal(data, &observation); err != nil {
		return fmt.Errorf("decode host-key observation: %w", err)
	}
	if err := app.ConfirmHostKey(ctx, managementstate.TargetID(*targetID), observation); err != nil {
		return fmt.Errorf("confirm explicitly approved host key: %w", err)
	}
...
func enroll(args []string) error {
	...
	for attempt := 1; ; attempt++ {
		...
		if errors.Is(lastErr, targetgateway.ErrTrustRequired) {
			return lastErr
		}

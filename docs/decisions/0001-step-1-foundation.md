...
The build receives version, Git revision, and `SOURCE_DATE_EPOCH` explicitly. Go builds use `-trimpath` and `-buildvcs=false`. Docker BuildKit receives `SOURCE_DATE_EPOCH`; Podman additionally receives `--timestamp`. CI checks reproducible image IDs independently for both engines.

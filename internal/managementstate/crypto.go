...
func (s SecretMaterial) MarshalJSON() ([]byte, error) { return nil, errors.New("secret material cannot be serialized") }

func encryptSecret(key []byte, id SecretID, plaintext []byte) ([]byte, error) {
	block, err := aes.NewCipher(key)
	if err != nil { return nil, fmt.Errorf("initialize secret cipher: %w", err) }
	gcm, err := cipher.NewGCM(block)
	if err != nil { return nil, fmt.Errorf("initialize secret gcm: %w", err) }
	nonce := make([]byte, gcm.NonceSize())
	if _, err := io.ReadFull(rand.Reader, nonce); err != nil { return nil, fmt.Errorf("generate secret nonce: %w", err) }
...
func decryptSecret(key []byte, id SecretID, ciphertext []byte) ([]byte, error) {
	if len(ciphertext) < 1 || ciphertext[0] != secretFormatVersion { return nil, errors.New("unsupported secret ciphertext format") }
	block, err := aes.NewCipher(key)
	if err != nil { return nil, errors.New("initialize secret cipher") }
	gcm, err := cipher.NewGCM(block)
	if err != nil { return nil, errors.New("initialize secret gcm") }
	if len(ciphertext) < 1+gcm.NonceSize()+gcm.Overhead() { return nil, errors.New("invalid secret ciphertext") }
	...
	if err != nil { return nil, errors.New("secret authentication failed") }

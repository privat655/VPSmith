...
func newID(prefix string) (string, error) {
	var raw [16]byte
	if _, err := rand.Read(raw[:]); err != nil { return "", fmt.Errorf("generate %s id: %w", prefix, err) }
...
type SwapDesiredState struct { Mode string `json:"mode,omitempty"`; SizeGiB int `json:"size_gib,omitempty"` }
...
func (s *Snapshot) normalize() {
	...
	if s.Sources.Artifacts == nil { s.Sources.Artifacts = []SourceArtifact{} }
	if s.Sources.Workspaces == nil { s.Sources.Workspaces = []SourceWorkspace{} }
...
func validBackupType(value BackupArtifactType) bool {
	switch value {
	case BackupRecoveryPackage, BackupCore, BackupModule, BackupSystemRestorePoint: return true

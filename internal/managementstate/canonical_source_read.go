...
func (s *Store) readCanonicalSources(ctx context.Context) (SourceState, error) {
	...
	if err != nil {
		return out, fmt.Errorf("read source artifacts: %w", err)
	}
	for rows.Next() {
	...
	if err != nil {
		return out, fmt.Errorf("read source workspaces: %w", err)
	}
	for rows.Next() {
	...
	if err == nil {
		out.CustomModuleGithub = &cfg
	} else if !errors.Is(err, sql.ErrNoRows) {
		return out, fmt.Errorf("read custom module github configuration: %w", err)
	}
	if out.Artifacts == nil {
		out.Artifacts = []SourceArtifact{}
	}
	if out.Workspaces == nil {
		out.Workspaces = []SourceWorkspace{}
	}
	return out, nil
}

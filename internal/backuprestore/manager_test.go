...
func TestLongTermEnvelopeAgeAndManifestContracts(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if strings.Contains(string(data), string(passphrase)) || strings.Contains(string(data), "secret-payload") { t.Fatal("encrypted artifact leaks plaintext") }
	inspection, err := manager.Inspect(ctx, artifact.Path, managementstate.BackupRecoveryPackage, passphrase)
	if err != nil { t.Fatal(err) }
	if err := inspection.Close(); err != nil { t.Fatal(err) }
	if _, err := manager.Inspect(ctx, artifact.Path, managementstate.BackupCore, passphrase); err == nil { t.Fatal("wrong artifact type accepted") }
	if _, err := manager.Inspect(ctx, artifact.Path, managementstate.BackupRecoveryPackage, []byte("wrong")); err == nil { t.Fatal("wrong passphrase accepted") }
}

func TestSystemRestorePointIsSeparateUnencryptedType(t *testing.T) {
	ctx := context.Background()
	state, err := managementstate.NewMemory()
	if err != nil { t.Fatal(err) }
	defer state.Close()
	...
	if err != nil { t.Fatal(err) }
	if filepath.Ext(artifact.Path) == ".age" { t.Fatal("system restore point was encrypted") }
	for _, name := range []string{"manifest.yaml", "storage.tar.zst", "SHA256SUMS"} {
		if _, err := os.Stat(filepath.Join(artifact.Path, name)); err != nil { t.Fatal(err) }
	}
}

...
func TestEmbeddedSnapshotsAndWorkspacesStaySeparated(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(snaps) != 3 { t.Fatalf("got %d embedded snapshots", len(snaps)) }
	for _, snap := range snaps {
		if snap.Version == "" || snap.SHA256 == "" { t.Fatalf("incomplete immutable identity: %#v", snap) }
	}
	...
	if err != nil { t.Fatal(err) }
	if ws.SHA256 == original { t.Fatal("workspace edit did not change local identity") }
	state, err := store.Sources(ctx)
	if err != nil { t.Fatal(err) }
	for _, a := range state.Artifacts {
		if a.ID == core.ID && a.SHA256 != original { t.Fatal("workspace mutated immutable base snapshot") }
	}
	diff, err := lib.Diff(ctx, ws.ID)
	if err != nil { t.Fatal(err) }
	if len(diff.Changes) == 0 { t.Fatal("expected structured diff") }
}
...
func TestCustomModuleLoadPushAndRemoteDrift(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if snap.Commit == "" || snap.SHA256 == "" { t.Fatalf("loaded source lacks exact identity: %#v", snap) }
	ws, err := lib.CreateWorkspace(ctx, snap.ID)
	if err != nil { t.Fatal(err) }
	if ws.BaseCommit != snap.Commit { t.Fatal("workspace lost base_commit") }
	...
	if err != nil { t.Fatal(err) }
	if pushed.SynchronizedCommit == "" { t.Fatal("successful push was not verified as synchronized") }
	remoteHead := git(t, "", "--git-dir", remotePath, "rev-parse", "refs/heads/main")
	if remoteHead != pushed.SynchronizedCommit { t.Fatalf("remote=%s synchronized=%s", remoteHead, pushed.SynchronizedCommit) }
	...
	if err != nil { t.Fatal(err) }
	competingCommit(t, seed, "remote change")
	_, err = lib.PushCustomModule(ctx, driftWS.ID, "local change")
	var drift *RemoteDriftError
	if !errors.As(err, &drift) { t.Fatalf("expected fail-closed remote drift, got %v", err) }
}
...
func TestPushRaceFailsClosedAndNeverMarksSynchronized(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	remote.beforePush = func() { competingCommit(t, seed, "race winner") }
	_, err = lib.PushCustomModule(ctx, ws.ID, "race loser")
	var drift *RemoteDriftError
	if !errors.As(err, &drift) { t.Fatalf("expected race drift, got %v", err) }
	state, err := store.Sources(ctx)
	if err != nil { t.Fatal(err) }
	for _, w := range state.Workspaces {
		if w.ID == ws.ID && w.SynchronizedCommit != "" { t.Fatal("failed race was marked synchronized") }
	}
}
...
func TestCoreThreeWayMergeProducesCandidateOrExplicitConflict(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if merged.Candidate == nil || len(merged.Conflicts) != 0 { t.Fatalf("expected clean candidate: %#v", merged) }
	if merged.Candidate.BaseSnapshotID != newSnap.ID { t.Fatal("clean merge candidate must reference new embedded base") }
	if ws.BaseSnapshotID != old.ID { t.Fatal("original core workspace base changed") }
	...
	if err != nil { t.Fatal(err) }
	if conflicted.Candidate != nil || len(conflicted.Conflicts) == 0 { t.Fatalf("expected explicit unresolved conflict: %#v", conflicted) }
}
...
func (*fakeRemote) Fetch(context.Context, RemoteConfig, string) (FetchResult, error) { return FetchResult{}, errors.New("not configured") }
func (*fakeRemote) Push(context.Context, PushRequest) (PushResult, error) { return PushResult{}, errors.New("not configured") }
...
func configureRemote(t *testing.T, ctx context.Context, store *managementstate.Store) {
	...
	err := store.Change(ctx, func(c *managementstate.Change) error {
		...
		if err := c.SetSecret(id, []byte("test-token-not-used-by-local-remote")); err != nil { return err }
...
func newBareRemote(t *testing.T) (string, string) {
	...
	if err := os.MkdirAll(filepath.Join(seed, "modules", "demo"), 0o755); err != nil { t.Fatal(err) }
...
func competingCommit(t *testing.T, seed, msg string) {
	t.Helper()
...
func findKind(t *testing.T, s []Snapshot, k managementstate.SourceKind) Snapshot {
	t.Helper()
	for _, v := range s { if v.Kind == k { return v } }
...
func tree(t *testing.T, content string) string {
	t.Helper()
	d := t.TempDir()
	if err := os.WriteFile(filepath.Join(d, "file.txt"), []byte(content), 0o644); err != nil { t.Fatal(err) }
	return d
}

...
func New(state *managementstate.Store, gateway *targetgateway.Gateway, compiler *deployment.Compiler, sources sourceLibrary) (*Coordinator, error) {
	if state == nil || gateway == nil || compiler == nil || sources == nil {
		return nil, errors.New("management state, target gateway, deployment compiler, and source library are required")
	}
...
func (c *Coordinator) PrepareNewTarget(ctx context.Context, req NewTargetRequest) (PreparedTarget, error) {
	...
	if err != nil {
		return PreparedTarget{}, fmt.Errorf("resolve released Cloud-init source: %w", err)
	}

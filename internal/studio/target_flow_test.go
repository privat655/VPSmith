...
func (f *fakeTargetApplication) PrepareNewTarget(context.Context, bootstrap.NewTargetRequest) (bootstrap.PreparedTarget, error) {
	return bootstrap.PreparedTarget{}, errors.New("unused")
}
func (f *fakeTargetApplication) SetTargetAddress(context.Context, managementstate.TargetID, string) error {
	f.addressCalls++
	return nil
}
func (f *fakeTargetApplication) ObserveHostKey(context.Context, managementstate.TargetID) (targetgateway.HostKeyObservation, error) {
	f.observeCalls++
...
func (f *fakeTargetApplication) ConfirmHostKey(_ context.Context, _ managementstate.TargetID, got targetgateway.HostKeyObservation) error {
	f.confirmCalls++
	if got != f.observation {
		return errors.New("different observation")
	}
	f.trusted = true
	return nil
}
func (f *fakeTargetApplication) Enroll(context.Context, managementstate.TargetID) (targetgateway.EnrollmentResult, error) {
	f.enrollCalls++
	if !f.trusted {
		return targetgateway.EnrollmentResult{}, targetgateway.ErrTrustRequired
	}
...
func TestTOFUObservationCannotImplicitlyConfirmOrEnroll(t *testing.T) {
	...
	if observeResult.Code != http.StatusOK {
		t.Fatalf("observe status=%d body=%s", observeResult.Code, observeResult.Body.String())
	}
	if app.observeCalls != 1 || app.addressCalls != 1 || app.confirmCalls != 0 || app.trusted {
		t.Fatalf("observe mutated trust: %#v", app)
	}
	...
	if enrollResult.Code != http.StatusPreconditionRequired || app.confirmCalls != 0 {
		t.Fatalf("enroll before confirm status=%d confirmCalls=%d", enrollResult.Code, app.confirmCalls)
	}
	...
	if confirmResult.Code != http.StatusOK || app.confirmCalls != 1 || !app.trusted {
		t.Fatalf("explicit confirm failed status=%d calls=%d", confirmResult.Code, app.confirmCalls)
	}
	...
	if enrollAfterResult.Code != http.StatusOK {
		t.Fatalf("enroll after confirm status=%d body=%s", enrollAfterResult.Code, enrollAfterResult.Body.String())
	}
}

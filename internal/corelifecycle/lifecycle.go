...
func desiredCoreForOperation(kind deployment.OperationKind, req PrepareRequest, current managementstate.CoreDesiredState, backup *verifiedBackup) (managementstate.CoreDesiredState, error) {
	...
	case deployment.Update, deployment.Validate:
		...
		if err := completeCoreDesiredRuntime(current); err != nil {
	...
	case deployment.Reconfigure:
		...
		if req.Configuration.Domain != "" {
			current.Domain = req.Configuration.Domain
		}
		if req.Configuration.ACMEEmail != "" {
			current.ACMEEmail = req.Configuration.ACMEEmail
		}
		if coreSecretRefsAny(req.Configuration.Secrets) {
			current.Secrets = req.Configuration.Secrets
		}
		if req.Configuration.Authelia.Enrollment != "" || len(req.Configuration.Authelia.Users) > 0 || len(req.Configuration.Authelia.Groups) > 0 {
			current.Authelia = normalizeDesiredAuthelia(req.Configuration.Authelia)
		}
		if req.Swap.Mode != "" {
...
func coreStateFromBackup(inspection *backuprestore.Inspection) (managementstate.CoreDesiredState, coreBackupImageLocks, error) {
	...
	if len(locks.Images) != 2 {
		return managementstate.CoreDesiredState{}, coreBackupImageLocks{}, errors.New("Core backup image lock set is incomplete")
	}
	if desired.Images == nil {
		desired.Images = map[string]managementstate.CoreImageIdentity{}
	}
	for _, name := range []string{"caddy", "authelia"} {
		...
		canonical, exists := desired.Images[name]
		if exists && (canonical.Ref != image.Ref || canonical.Digest != image.Digest) {
			return managementstate.CoreDesiredState{}, coreBackupImageLocks{}, fmt.Errorf("Core backup canonical %s image identity disagrees with image locks", name)
		}
		desired.Images[name] = managementstate.CoreImageIdentity{Ref: image.Ref, Digest: image.Digest}
	}
...
func candidateForOperation(kind deployment.OperationKind, req PrepareRequest, observed managementstate.ObservedState, backup *verifiedBackup) (sourcelibrary.CoreCandidateRef, error) {
	...
	case deployment.Install, deployment.Update:
		if req.Candidate.WorkspaceID != "" {
			return sourcelibrary.CoreCandidateRef{}, errors.New("mutable Core workspace must be explicitly adopted before target planning")
		}
...
func deploymentCoreSecrets(refs managementstate.CoreSecretReferences) deployment.CoreSecretIDs {
	return deployment.CoreSecretIDs{AutheliaSession: string(refs.AutheliaSession), AutheliaStorage: string(refs.AutheliaStorage), AutheliaResetPassword: string(refs.AutheliaResetPassword), AutheliaUsersDatabase: string(refs.AutheliaUsersDatabase)}
}
...
func validatePostState(prepared Prepared, observed managementstate.ObservedState) error {
	...
	if !observed.Core.HTTPS {
		return errors.New("Core HTTPS endpoint is not valid")
	}
	if err := requireExactRunningCoreImages(prepared.DesiredCore.Images, observed.Core.Containers); err != nil {
		return err
	}
	if err := requirePublicRoutes(prepared.Operation.PublicRoutes, observed.Core.PublicRoutes); err != nil {
		return err
	}
	return nil
}
func requireExactRunningCoreImages(desired map[string]managementstate.CoreImageIdentity, containers []managementstate.ContainerObservedState) error {
	if len(desired) != 2 {
		return errors.New("canonical Core desired image locks are incomplete")
	}
	byName := make(map[string]managementstate.ContainerObservedState, len(containers))
	...
	for _, name := range []string{"caddy", "authelia"} {
		...
		if !ok || !present || !got.Present || !got.Running || got.ImageDigest != want.Digest {
			return fmt.Errorf("running %s image does not match approved digest", name)
		}
		if got.ImageRef != "" && got.ImageRef != want.Ref && got.ImageRef != want.Ref+"@"+want.Digest {
			return fmt.Errorf("running %s image reference does not match approved identity", name)
		}
	}
	return nil
}
func requirePublicRoutes(expected []deployment.CorePublicRoute, observed []managementstate.PublicRouteObservedState) error {
	if len(expected) != len(observed) {
		return errors.New("observed public Core routes do not match the approved platform routes")
	}
	byKey := make(map[string]managementstate.PublicRouteObservedState, len(observed))
	for _, route := range observed {
		key := route.Hostname + "\x00" + route.PathPrefix
		if _, duplicate := byKey[key]; duplicate {
			return fmt.Errorf("public route %s%s was observed more than once", route.Hostname, route.PathPrefix)
		}
		byKey[key] = route
	}
	for _, want := range expected {
		got, ok := byKey[want.Hostname+"\x00"+want.PathPrefix]
		if !ok || got.AuthMode != want.AuthMode || !got.HTTPS {
			return fmt.Errorf("public route %s%s is not healthy over HTTPS", want.Hostname, want.PathPrefix)
		}
		if want.AuthMode == "protected" && !got.AuthEnforced {
			return fmt.Errorf("public route %s%s does not enforce Authelia", want.Hostname, want.PathPrefix)
		}
	}
	return nil
}
func coreDNSHostnames(domain string, routes []deployment.CorePublicRoute) []string {
	seen := map[string]struct{}{"auth." + domain: {}}
	out := []string{"auth." + domain}

...
func requireCoreBackupReady(snapshot managementstate.Snapshot, target managementstate.Target, observed managementstate.ObservedState) error {
	...
	if err := completeCoreDesiredRuntime(desired); err != nil {
		return fmt.Errorf("Core backup requires complete canonical desired state: %w", err)
	}
	...
	if err := requireObservedCoreImages(desired.Images, observed.Core.Containers); err != nil {
		return fmt.Errorf("Core backup exact image validation failed: %w", err)
	}
	if err := requireObservedPublicRoutesHealthy(observed.Core.PublicRoutes); err != nil {
		return fmt.Errorf("Core backup public route validation failed: %w", err)
	}
	return nil
}
...
func requireObservedCoreImages(expected map[string]managementstate.CoreImageIdentity, containers []managementstate.ContainerObservedState) error {
	for _, name := range []string{"caddy", "authelia"} {
		...
		if !ok {
			return fmt.Errorf("missing desired %s image lock", name)
		}
		...
		for i := range containers {
			...
			if match != nil {
				return fmt.Errorf("ambiguous observed %s container", name)
			}
			match = &containers[i]
		}
		if match == nil || !match.Present || !match.Running {
			return fmt.Errorf("%s container is not running", name)
		}
		if match.ImageDigest != want.Digest {
			return fmt.Errorf("%s running image digest does not match canonical desired state", name)
		}
		if match.ImageRef != "" && match.ImageRef != want.Ref && match.ImageRef != want.Ref+"@"+want.Digest {
			return fmt.Errorf("%s running image reference does not match canonical desired state", name)
		}
	}
	return nil
}
...
func requireObservedPublicRoutesHealthy(routes []managementstate.PublicRouteObservedState) error {
	for _, route := range routes {
		if strings.TrimSpace(route.Hostname) == "" || strings.TrimSpace(route.PathPrefix) == "" || !route.HTTPS {
			return errors.New("public route observation is incomplete or not HTTPS")
		}
		if route.StatusCode < 200 || route.StatusCode >= 500 || route.StatusCode == 404 {
			return fmt.Errorf("public route %s%s is not healthy", route.Hostname, route.PathPrefix)
		}
		if route.AuthMode == "protected" && !route.AuthEnforced {
			return fmt.Errorf("protected public route %s%s does not enforce Authelia", route.Hostname, route.PathPrefix)
		}
	}
	return nil
}
